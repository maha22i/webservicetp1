...
export class EquipeService {
  constructor(
    private mapper: EquipeMapper,
    @InjectModel(EquipeEntity.name) private model: Model<EquipeDocument>
  ) {}

  create(dto: EquipeCreateDto): Observable<EquipeDto> {
  ...
  findAll(skip: number, limit: number): Observable<PaginatedItems<EquipeDto>> {
  ...
  findOne(id: string): Observable<EquipeDto> {
  ...
  update(dto: EquipeUpdateDto): Observable<EquipeDto> {
  ...
  reset(dto: EquipeResetDto): Observable<EquipeDto> {
